...
The plugin export the symbols (for the moment only functions) recognized by IDA Pro to the ELF symbol table. This allows us to use the power of IDA in recognizing functions (analysis, FLIRT signatures, manual creation, renaming, etc), but not be limited to the exclusive use of this tools.
...
Rename some functions in IDA, run `syms2elf` and select the output file.
